...
public class Exam0510 {
  public static void main(String[] args) {
    int[] p = new int[3]; // c언어로 표현한다면 => (int*)malloc(sizeof(int) * 3)와 같다.
    // p
    // - int 배열의 주소를 담는 변수이다.
    // - 보통 '레퍼런스'라 부른다.
    //
    ...
    // p[-1] = 500; // 실행 예외 발생!(index의 범위를 벗어남)
    
    int sum = 0;
    for (int i = 0; i < p.length; i++) { // 0부터 배열 길이전까지
      sum = sum + p[i];
    }
    ...
    //Systme.out.println(p3[1]); // 컴파일 오류!
    ...
    // String str = "okok"; // 요것도 레퍼런스 변수이다.
    
    // 레퍼런스 변수의 주소값을 0으로 초기화하기
    //p2 = o; // 컴파일 오류! 이렇게 직접적으로 초기화시킬 수 없다.
    p2 = null; // 주소 변수(레퍼런스)를 0으로 초기화 시키려면 null을 사용하라.
    
    //System.out.print(p2[0]); // 컴파일은 진행되나 주소값이 초기화돼서 실행이 안됨
    ...
    System.gc(); // 가능하다면 빠른 시일 ㅈ내에 가비지를 수집해 줄 것을 요구!
    // 주의
